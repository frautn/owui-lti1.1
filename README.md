...
Autologin needs cookies for the same site, so Django and Open WebUI must be in the same domain. Open WebUI works at root `/` and this is hardcoded, so we need to move Django to a subfolder. This app is is coded to work under `/django/`.

- Launch URL: `https://your.domain/django/lti/launch/`
- Config URL (cartridge XML): `https://your.domain/django/lti/config.xml`
- Chat page (session-backed): `https://your.domain/django/lti/chat/`
- Health check: `https://your.domain/django/lti/health/`

## Moodle Configuration (LTI 1.1)

In Moodle external tool settings:

- Tool URL: your Django launch URL, for example `https://your-host/django/lti/launch/`
- Consumer key: `MOODLE_CONSUMER_KEY`
- Shared secret: `MOODLE_SHARED_SECRET`

You can also import the tool using the cartridge config URL: `https://your-host/django/lti/config.xml`.

## Tested in these Moodle versions
  - 5.02
  - 3.?? (Major version 3, unkown subversion)